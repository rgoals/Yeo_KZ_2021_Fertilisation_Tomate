
# Hauteur des plants

```{r message = FALSE, warning = FALSE, echo = FALSE}
library(tidyverse)
library(ggpubr)
library(rstatix)
```

## Préparation des données

Jeu de données `veg` qui contient différentes mesures dont les hauteurs de plants de tomate, mesurées 4 fois en cours de culture, lors d'un essai de fertilisation de fin novembre 2020 à mars 2021. Dispositif expérimental en bloc de Fisher avec 6 traitements en 5 répétitions :

- `T0` : Témoin sans engrais
- `T1` : Fumier porcs
- `T2` : Fumier poulets
- `T3` : Engrais mineral
- `T4` : Combinaison 50% Fumier de Porcs + 50% Engrais Minéral
- `T5` : Combinaison 50% Fumier de Poulets + 50% Engrais Minéral

Une hauteur moyenne est calculée pour les 4 plants centraux mesurés le même jour de la semaine en 4 séances : 

1. `haut1` : 55 j AS, 21 j AR, semaine 3 AR
1. `haut2` : 69 j AS, 35 j AR, semaine 5 AR 
1. `haut3` : 76 j AS, 42 j AR, semaine 6 AR 
1. `haut4` : 83 j AS, 49 j AR, semaine 7 AR.

La question est de savoir si les différents types d'engrais induisent des augmentations de hauteur de plant significativement différentes. 

> **Nous souhaitons évaluer si ces différents engrais apportent une plus value comparée au témoin; surtout si les effets diffèrent d'un type d'engrais à l'autre sur les paramètres végétatifs et/ou de rendement.**

Charger le dataframe et extraire seulement les variables d'intérêt pour l'analyse de la hauteur des plants.

```{r}
df <- read_csv("data/veg.csv")
```

```{r}
df <- df %>% 
  mutate(id = 1:nrow(.), .before = 1) %>%
  select(id, bloc, traitement, engrais, haut1, haut2, haut3, haut4) %>% 
  convert_as_factor(traitement, engrais)

df$engrais <- factor(df$engrais, 
                     levels = c("Temoin", "Fumier porcs", "Fumier poulets", 
                                "Engrais mineral", "50% FPorcs + 50% Eng. Min.", 
                                "50% FPoulets + 50% Eng. Min."))

write_csv(df, "tab/hauteurs.csv")
df_original <- df # une copie

set.seed(123)
df %>% sample_n_by(bloc, size = 1)
```

## Hauteur 1

### Visualisation, boxplots

```{r}
bxp <- ggplot(df, aes(x = engrais, y = haut1)) +
  geom_boxplot() +
  theme_classic() +
  xlab("Engrais") + ylab("Hauteur des plants - cm") +
  theme(axis.text.x = element_text(angle = 45, color = "black", vjust = 1, hjust = 1))
bxp
```

### Identification des outliers extrêmes

```{r}
df_out <- df %>%
  group_by(engrais) %>%
  identify_outliers(haut1) %>% 
  select(engrais, bloc, haut1, is.outlier, is.extreme)
df_out
```

Il n'y a pas d'observation aberrante extrême.

```{r}
#df <- df %>% filter(! id %in% df_out$id)
```


### Le modèle ANOVA

```{r}
lm1 <- lm(haut1 ~ engrais, data = df)
Anova(lm1)
```

À la semaine 3 après le repiquage, il ne serait pas encore possible d'établir des différences significatives entre les effets des traitements sur les hauteurs des plants. Mais vérifions les conditions de validité de l'ANOVA.

### Indépendance des résidus

```{r}
plot(lm1, 1)
```

### Normalité des résidus

```{r}
plot(lm1, 2)
```

```{r}
shapiro_test(residuals(lm1))
```

### Homogénéité des variances

```{r}
plot(lm1, 3)
```

```{r}
levene_test(residuals(lm1) ~ engrais, data = df)
```

On retient que la conclusion de l'ANOVA peut être considérée valide : => Pas de différence d'effet sur la hauteur des plants à cette date. 

### Comparaisons par paires

Tests post-hoc de multiples comparaisons par paires entre les groupes (ici, les engrais) pour identifier ceux dont les effets sont différents.

```{r}
cm <- multcomp::glht(lm1, linfct = multcomp::mcp(engrais = "Tukey"))
#summary(cm)
```

### Graph des comparaisons

```{r}
#par(mar = c(0, 22, 0, 0))
#plot(cm)
```

### Lettres pour différencier

```{r}
cld <- multcomp::cld(cm)
cld
```

### Les lettres dans un dataframe pour ggplot

```{r}
Le <- cld$mcletters$Letters
L <- data.frame(engrais = levels(df$engrais),
                lettres = Le)
L
```

### Associer les moyennes et les écart-types pour le rapport

```{r}
haut1_moy <- df %>% 
  group_by(engrais) %>% 
  get_summary_stats(haut1, type = "mean_sd") %>%
  select(-variable) %>% 
  mutate(lettres = L$lettres)
write_csv(haut1_moy, "tab/hauteur1_moyennes.csv")
haut1_moy
```

### Graphe avec les comparaisons

```{r}
ggplot(haut1_moy, aes(x = engrais, y = mean)) + 
  geom_bar(stat = "identity", color = "blue", fill = "grey", width = 0.6) +
  ylim(0, 25) +
  theme_bw() +
  xlab("Engrais") + ylab("Hauteur des plants - cm") +
  theme(axis.text.x = element_text(angle = 45, color = "black", vjust = 1, hjust = 1)) +
  geom_text(aes(label = lettres), vjust = -0.5, size = 4)
```


## Hauteur 2

```{r}
df <- df_original
```

### Visualisation, boxplots

```{r}
bxp <- ggplot(df, aes(x = engrais, y = haut2)) +
  geom_boxplot() +
  theme_classic() +
  xlab("Engrais") + ylab("Hauteur des plants - cm") +
  theme(axis.text.x = element_text(angle = 45, color = "black", vjust = 1, hjust = 1))
bxp
```

### Identification des outliers extrêmes

```{r}
df_out <- df %>%
  group_by(engrais) %>%
  identify_outliers(haut2) %>% 
  select(engrais, bloc, haut2, is.outlier, is.extreme)
df_out
```

Il n'y a pas d'observation aberrante extrême.

```{r}
#df <- df %>% filter(! id %in% df_out$id)
```


### Le modèle ANOVA

```{r}
lm2 <- lm(haut2 ~ engrais, data = df)
Anova(lm2)
```

À la semaine 5 après le repiquage, au moins deux types d'engrais ont des effets très significativement différents sur les hauteurs des plants. Mais vérifions les conditions de validité de l'ANOVA.

### Indépendance des résidus

```{r}
plot(lm2, 1)
```

### Normalité des résidus

```{r}
plot(lm2, 2)
```

```{r}
shapiro_test(residuals(lm2))
```

### Homogénéité des variances

```{r}
plot(lm2, 3)
```

```{r}
levene_test(residuals(lm2) ~ engrais, data = df)
```

On retient que la conclusion de l'ANOVA peut être considérée valide. Trouvons les engrais dont les effets sont différents.

### Comparaisons par paires

Tests post-hoc de multiples comparaisons par paires entre les groupes (ici, les engrais) pour identifier ceux dont les effets sont différents.

```{r}
cm <- multcomp::glht(lm2, linfct = multcomp::mcp(engrais = "Tukey"))
summary(cm)
```

### Graph des comparaisons

```{r}
par(mar = c(0, 22, 0, 0))
plot(cm)
```

### Lettres pour différencier

```{r}
cld <- multcomp::cld(cm)
cld
```

### Les lettres dans un dataframe pour ggplot

```{r}
Le <- cld$mcletters$Letters
L <- data.frame(engrais = levels(df$engrais),
                lettres = Le)
L
```

### Associer les moyennes et les écart-types pour le rapport

```{r}
haut2_moy <- df %>% 
  group_by(engrais) %>% 
  get_summary_stats(haut2, type = "mean_sd") %>%
  select(-variable) %>% 
  mutate(lettres = L$lettres)
write_csv(haut2_moy, "tab/hauteur2_moyennes.csv")
haut2_moy
```

### Graphe avec les comparaisons

```{r}
ggplot(haut2_moy, aes(x = engrais, y = mean)) + 
  geom_bar(stat = "identity", color = "blue", fill = "grey", width = 0.6) +
  ylim(0, 40) +
  theme_bw() +
  xlab("Engrais") + ylab("Hauteur des plants - cm") +
  theme(axis.text.x = element_text(angle = 45, color = "black", vjust = 1, hjust = 1)) +
  geom_text(aes(label = lettres), vjust = -0.5, size = 4)
```


## Hauteur 3

```{r}
df <- df_original
```

### Visualisation, boxplots

```{r}
bxp <- ggplot(df, aes(x = engrais, y = haut3)) +
  geom_boxplot() +
  theme_classic() +
  xlab("Engrais") + ylab("Hauteur des plants - cm") +
  theme(axis.text.x = element_text(angle = 45, color = "black", vjust = 1, hjust = 1))
bxp
```

### Identification des outliers extrêmes

```{r}
df_out <- df %>%
  group_by(engrais) %>%
  identify_outliers(haut3) %>% 
  select(engrais, bloc, haut3, is.outlier, is.extreme)
df_out
```

Il n'y a pas d'observation aberrante extrême.

```{r}
#df <- df %>% filter(! id %in% df_out$id)
```


### Le modèle ANOVA

```{r}
lm3 <- lm(haut3 ~ engrais, data = df)
Anova(lm3)
```

À la semaine 6 après le repiquage, au moins deux types d'engrais ont des effets très significativement différents sur les hauteurs des plants. Mais vérifions les conditions de validité de l'ANOVA.

### Indépendance des résidus

```{r}
plot(lm3, 1)
```

### Normalité des résidus

```{r}
plot(lm3, 2)
```

```{r}
shapiro_test(residuals(lm3))
```

### Homogénéité des variances

```{r}
plot(lm3, 3)
```

```{r}
levene_test(residuals(lm3) ~ engrais, data = df)
```

On retient que la conclusion de l'ANOVA peut être considérée valide. Trouvons les engrais dont les effets sont différents.

### Comparaisons par paires

Tests post-hoc de multiples comparaisons par paires entre les groupes (ici, les engrais) pour identifier ceux dont les effets sont différents.

```{r}
cm <- multcomp::glht(lm3, linfct = multcomp::mcp(engrais = "Tukey"))
summary(cm)
```

### Graph des comparaisons

```{r}
par(mar = c(0, 22, 0, 0))
plot(cm)
```

### Lettres pour différencier

```{r}
cld <- multcomp::cld(cm)
cld
```

### Les lettres dans un dataframe pour ggplot

```{r}
Le <- cld$mcletters$Letters
L <- data.frame(engrais = levels(df$engrais),
                lettres = Le)
L
```

### Associer les moyennes et les écart-types pour le rapport

```{r}
haut3_moy <- df %>% 
  group_by(engrais) %>% 
  get_summary_stats(haut3, type = "mean_sd") %>%
  select(-variable) %>% 
  mutate(lettres = L$lettres)
write_csv(haut3_moy, "tab/hauteur3_moyennes.csv")
haut3_moy
```

### Graphe avec les comparaisons

```{r}
ggplot(haut3_moy, aes(x = engrais, y = mean)) + 
  geom_bar(stat = "identity", color = "blue", fill = "grey", width = 0.6) +
  ylim(0, 40) +
  theme_bw() +
  xlab("Engrais") + ylab("Hauteur des plants - cm") +
  theme(axis.text.x = element_text(angle = 45, color = "black", vjust = 1, hjust = 1)) +
  geom_text(aes(label = lettres), vjust = -0.5, size = 4)
```


## Hauteur 4

```{r}
df <- df_original
```

### Visualisation, boxplots

```{r}
bxp <- ggplot(df, aes(x = engrais, y = haut4)) +
  geom_boxplot() +
  theme_classic() +
  xlab("Engrais") + ylab("Hauteur des plants - cm") +
  theme(axis.text.x = element_text(angle = 45, color = "black", vjust = 1, hjust = 1))
bxp
```

### Identification des outliers extrêmes

```{r}
df_out <- df %>%
  group_by(engrais) %>%
  identify_outliers(haut4) %>% 
  select(engrais, bloc, haut4, is.outlier, is.extreme)
df_out
```

Il n'y a pas d'observation aberrante extrême.

```{r}
#df <- df %>% filter(! id %in% df_out$id)
```


### Le modèle ANOVA

```{r}
lm4 <- lm(haut4 ~ engrais, data = df)
Anova(lm4)
```

À la semaine 7 après le repiquage, au moins deux types d'engrais ont des effets très significativement différents sur les hauteurs des plants. Mais vérifions les conditions de validité de l'ANOVA.

### Indépendance des résidus

```{r}
plot(lm4, 1)
```

### Normalité des résidus

```{r}
plot(lm4, 2)
```

```{r}
shapiro_test(residuals(lm4))
```

### Homogénéité des variances

```{r}
plot(lm4, 3)
```

```{r}
levene_test(residuals(lm4) ~ engrais, data = df)
```

On retient que la conclusion de l'ANOVA peut être considérée valide. Trouvons les engrais dont les effets sont différents.

### Comparaisons par paires

Tests post-hoc de multiples comparaisons par paires entre les groupes (ici, les engrais) pour identifier ceux dont les effets sont différents.

```{r}
cm <- multcomp::glht(lm4, linfct = multcomp::mcp(engrais = "Tukey"))
summary(cm)
```

### Graph des comparaisons

```{r}
par(mar = c(0, 22, 0, 0))
plot(cm)
```

### Lettres pour différencier

```{r}
cld <- multcomp::cld(cm)
cld
```

### Les lettres dans un dataframe pour ggplot

```{r}
Le <- cld$mcletters$Letters
L <- data.frame(engrais = levels(df$engrais),
                lettres = Le)
L
```

### Associer les moyennes et les écart-types pour le rapport

```{r}
haut4_moy <- df %>% 
  group_by(engrais) %>% 
  get_summary_stats(haut4, type = "mean_sd") %>%
  select(-variable) %>% 
  mutate(lettres = L$lettres)
write_csv(haut4_moy, "tab/hauteur4_moyennes.csv")
haut4_moy
```

### Graphe avec les comparaisons

```{r}
ggplot(haut4_moy, aes(x = engrais, y = mean)) + 
  geom_bar(stat = "identity", color = "blue", fill = "grey", width = 0.6) +
  ylim(0, 50) +
  theme_bw() +
  xlab("Engrais") + ylab("Hauteur des plants - cm") +
  theme(axis.text.x = element_text(angle = 45, color = "black", vjust = 1, hjust = 1)) +
  geom_text(aes(label = lettres), vjust = -0.5, size = 4)
```





